...
export default {
    data () {
        return {
            loading: true, // прелоадер выключится после получения ответа сервера
            email: '',
            custPwd: '',
            checkPwd: '',
            isReg: undefined, // регистрация пользователя не определена
            badPwd: false, // выводится сообщение Неверный пароль
            hash: '',
    ...
    methods: {
        ...
        userAuth () {
            ...
            formData.append("book", this.$f7.data.book_id);
            formData.append("action", "auth");
            formData.append("email", this.email);
            formData.append("pwd", this.custPwd);
            this.$http
                .post(this.$f7.data.mainUrl+this.$f7.data.backend, formData)
                .then(response => {
    ...
    mounted () {
        ...
        let formData = {book: this.$f7.data.book_id, action: 'get_data', tbl: 'check_email', email: this.email}
